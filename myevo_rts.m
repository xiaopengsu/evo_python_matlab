...
[m,n]=size(mono);
for i=1:m 
           TimeS=mono(i,1)*ones(size(true,1),1);%guiding diyihang diyilie de shuju 
           [error,idxx]=min(abs(true(:,1)-TimeS));%na true zhong suoyou de shuju yu ta bijiao 
    
           idx=[idx,idxx]; %time yiyang duiying de hang shu

end
true_dq=true(idx,:);%baocun tiqu hou de shuju

true_dq_x=true_dq(:,2); true_dq_y=true_dq(:,3); true_dq_z=true_dq(:,4);
monox=mono(:,2); monoy=mono(:,3); monoz=mono(:,4);

gtx=true_dq_x-true_dq_x(1);
gty=true_dq_y-true_dq_y(1);
gtz=true_dq_z-true_dq_z(1);
...
%保存时间戳对齐的文件
%gt_t=true(:,1);gt_x=true(:,2);gt_y=true(:,3);gt_z=true(:,4);gt_w=true(:,5);gt_q_x=true(:,6);gt_q_y=true(:,7);gt_q_z=true(:,8);
...
% after R plot
figure(2);
% plot3(true_dq_x,true_dq_y,true_dq_z,'r');
plot3(gtx,gty,gtz,'ro');
grid on;
hold on;
